...
/**
 * Client to query {@link RentalCarAPI} and parse the 
...
public class RentalCarApiClient {
  ...
  /**
   * Sends a http GET request to {@link RentalCarAPI}
   * to get a rental Car. 
   */
  public RentalCar getRentalCar(String carClass, Location origin) throws IOException {
    
    /* 
     * By default app runs locally at port 8080. In case if you define
     * the running port, change the port in following URL accordingly
     */
    
    /* Prepare url in same format as defined in RentalCarAPI
     * i.e. http://localhost:8080/rentalCarApi/getRentalCar/{Car Class}/{source Latitude}/{source Longitude}
     * where {} are our inputs
     */
    URL url =
        new URL("http://localhost:8080/rentalCarApi/getRentalCar/" + carClass
  ...
  public RentalCarTrip rentalCarTrip(String carClass, Location origin, Location destination) throws IOException {
     ...
     *  Hint - See format of makeTrip call in RentalCarAPI
